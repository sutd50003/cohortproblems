...
Consider the code `findmax.ts` given in the template project, write a testsuite using Jest which contains 3 test cases.
1. a test that results in failure (do not compute maximum) it should fail
1. a test that results in error, it should throw an error that the test won't catch this error
...
Consider the code `findmax.ts` from the previous question, define a list of test cases using equivalence class testing. 
...
1. arrays with more than one ordinal value and the max value is at index N-1, where N is the lengths.
